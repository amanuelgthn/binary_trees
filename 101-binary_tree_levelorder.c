...
**/
{
	...
	{
		binary_tree_print_level_order(node->left, level - 1, func);
		binary_tree_print_level_order(node->right, level - 1, func);
	}
}
/**
...
void binary_tree_levelorder(const binary_tree_t *tree, void (*func)(int))
{
	if (tree == NULL || func == NULL)
		return;
	int height = binary_tree_height(tree), i;

	for (i = 1; i <= height; i++)
		binary_tree_print_level_order(tree, i, func);
}
